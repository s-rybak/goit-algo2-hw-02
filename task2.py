...
def optimize_printing(print_jobs: List[Dict], constraints: Dict) -> Dict:
    ...
    printer_constraints = PrinterConstraints(
        max_volume=constraints["max_volume"], max_items=constraints["max_items"]
    )

    # Сортуємо завдання спочатку за пріоритетом і за часом друку
    ...
    while not all(used):
        ...
        for i, job in enumerate(jobs):
            ...
            if (
                ...
                group_max_time = max(group_max_time, job.print_time)
                used[i] = True

        # Додаємо групу в пулл завдань
        if current_group:
            print_order.append(current_group)
